...
public class ThemMoiController implements Initializable {
    ...
    public void create(ActionEvent event){
        if (validateValueInForm()) {
            ...
            temp.setTieuSu(tieuSuTxb.getText());
    ...
    public boolean addNewPeople(NhanKhauBean nhanKhauBean) throws SQLException, ClassNotFoundException{
        ...
        // 1 - 22
        String query = "INSERT INTO nhan_khau (hoTen, bietDanh, namSinh, gioiTinh, noiSinh, nguyenQuan, danToc, tonGiao, quocTich, soHoChieu, noiThuongTru, diaChiHienNay, trinhDoHocVan, TrinhDoChuyenMon, bietTiengDanToc, trinhDoNgoaiNgu, ngheNghiep, noiLamViec, idNguoiTao, ngayTao,tienAn,tieuSu,ghiChu)"
                + " values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";
        ...
        preparedStatement.setString(21, nhanKhau.getTienAn());
        preparedStatement.setString(22, nhanKhau.getTieuSu());
